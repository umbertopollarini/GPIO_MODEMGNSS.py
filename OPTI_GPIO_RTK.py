...
def init_udp():
    ...
    for host, port in CONFIG["destinations"]:
        ...
        # buffer più grande: meno drop quando la cella ha jitter
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 262144)
        # DSCP EF (expedited forwarding): se ignorato dall'operatore, non fa danni
        try:
            sock.setsockopt(socket.IPPROTO_IP, socket.IP_TOS, 0xB8)
        except OSError:
            pass
        # Connetti il socket alla destinazione: send() è più efficiente di sendto()
        try:
            sock.connect((host, port))
        except OSError as e:
            print(f"[UDP] connect fallita verso {host}:{port}: {e}")
        udp_socks.append((sock, host, port))
        print(f"[UDP] destinazione {host}:{port} (socket connesso)")


def send_udp(msg: str):
    """Invia msg a tutte le destinazioni UDP configurate (socket già connessi)."""
    data = msg.encode('ascii', errors='ignore')
    for sock, host, port in udp_socks:
        try:
            sock.send(data)
        except OSError as e:
            # se fallisce, tentiamo una sola riconnessione best-effort
            try:
                sock.connect((host, port))
                sock.send(data)
            except Exception as e2:
                print(f"[!] UDP error {host}:{port} – {e2}")
